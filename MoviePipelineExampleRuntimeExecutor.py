...
class MoviePipelineExampleRuntimeExecutor(unreal.MoviePipelinePythonHostExecutor):
    ...
    def _post_init(self):
        ...
        self.pipelineQueue=unreal.uproperty(unreal.MoviePipelineQueue)
        self.index=0
    ...
    def execute_delayed(self, inPipelineQueue):
        ...
        try:
            levelSequencePath ='/Game/NewLevelSequence.NewLevelSequence'
        ...
        self.pipelineQueue =unreal.load_asset(cmdParameters["asset_path"])
        ...
        i=0
        print(cmdParameters['custom_frames'])
        tensor=json.loads(cmdParameters['custom_frames'])
        self.pipeline_queue=self.pipelineQueue
        print("Length of queue after executing  job",len(self.pipeline_queue.get_jobs()))
        for job in self.pipelineQueue.get_jobs():
            ...
            outputSetting.set_editor_property('custom_end_frame',tensor[i][1])
            outputSetting.set_editor_property('custom_start_frame',tensor[i][0])
            print("The number of settings in the list are", len(x))
            i=i+1
        self.activeMoviePipeline.initialize(self.pipeline_queue.get_jobs()[self.index])
    ...
    def on_movie_pipeline_finished(self, inMoviePipeline, bSuccess):
        ...
        print("Value of index during on movie pipeline finished ",self.index)
        print("Length of queue after executing  job",len(self.pipeline_queue.get_jobs()))
        if(self.index==len(self.pipeline_queue.get_jobs())-1):
            print("Jos left are ",self.pipeline_queue)
            unreal.log("Finished rendering movie! Success: " + str(bSuccess))
            self.activeMoviePipeline = None
            self.on_executor_finished_impl()
        else:
            self.pipeline_queue.delete_job(self.pipeline_queue.get_jobs()[self.index])
